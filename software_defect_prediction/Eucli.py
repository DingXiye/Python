...
class Eucil(object):
    ...
    def fit_sample(self, data, label):
        ...
        l_ = len(bin1)
        mark = [1, 3, 7, 15, 31, 63]
        '''第一代生成l_个样本，第二代新生成2**1=2l_个，累积生成3l_个,第三代新生成2**2=4l_个,累积7个
            这里计算的是到每一代累积会生成的样本个数（l_的倍数）
        '''
        p = self.T / l_ #p是需要的倍数，配合上面的mark就可以知道需要多少代
        is_full = True 
        '''is_full 是否溢出，也即最后一代算完，是超出了T，还是未满T'''
        #但是因为毕竟生成的样本有偏差，避免生成过多，其实是宁愿少生成几个，也不愿意生成过多
    ...
    def generate_new_sample(self, bin1, bin2, g, l, k, is_full):
        ...
        if g >= 2 or (g == 1 and is_full is False):
            ...
            for i in range(l):
                # 生成子代
                # b1 = np.atleast_2d(bin1[i]) #把一个长度为len的一维数组，看做一个（1，len)大小的二维数组
                # b2 = np.atleast_2d(bin2[i])
                # b12 = np.append(b1,b2,axis= 0)
                # lv_0.append(np.mean(b12, axis=0))
